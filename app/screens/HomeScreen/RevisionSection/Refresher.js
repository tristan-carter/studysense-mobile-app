...
function Refresher({ navigation }) {
  ...
  const generateRound = () => {
    ...
    const allCards = currentSet.cards.filter((card) => card.totalIncorrect * 5 > card.totalCorrect);
  ...
  useEffect(() => {
    if (finishRoundReview) {
      ...
      const allCardsLearned = currentSet.cards.every((card) => card.totalIncorrect*4 < card.totalCorrect)

      if (allCardsLearned) {
        // show that they finished the set and allow them to restart
        console.log("all cards refreshed")
      }
      typedAnswer.current = "";
      ...
      <View style={styles.restartContainer}>
        <View style={styles.restartContainer}>
          <Image
            ...
            resizeMode="contain"
          />
          <Text style={styles.title}>Confident On Cards</Text>
          <Text style={styles.description}>
            We think you are confident on all the cards you have learned so far. Start learning more cards to do more refreshers!
          </Text>
        </View>
      </View>
    )
